 ...
 *
 */
function eie_theme_preprocess_html(&$vars) {
 ...
 *
 */
function eie_theme_preprocess_page(&$vars) {
  //Setting defaults
 ...
 */
function eie_theme_preprocess_node(&$vars, $hook) {

  $vars['theme_hook_suggestions'][] = 'node__' . $vars['type'] . '__' . $vars['view_mode'];

  if($vars['type'] == 'preparatory_lesson') {
    $product = _eie_theme_get_product_from_path();
...
*/
function eie_theme_preprocess_user_profile(&$vars) {
  global $user;
 ...
 */
function eie_theme_preprocess_entity(&$vars) {
 ...
 */
function eie_theme_preprocess_field(&$vars) {
 ...
 */
function eie_theme_views_pre_render(&$view) {
 ...
 */
function eie_theme_preprocess_views_view(&$vars) {
  $view = $vars['view'];

  if($view->name == 'resource') {
    if($view->current_display == 'additional_resources_block') {
      $short_code = _eie_theme_get_product_short_code();
 ...
 */
function eie_theme_preprocess_region(&$vars, $hook) {
 ...
 */
function eie_theme_preprocess_block(&$vars, $hook) {
...
// Utility Navigation
function eie_theme_menu_tree__menu_utility_navigation(&$vars) {
  print '<div class="row">';
...
// Footer Links
function eie_theme_menu_tree__menu_footer_links(&$vars) {
...
function eie_theme_menu_tree__menu_footer_links_2(&$vars) {
 ...
 */
function eie_theme_form_alter(&$form, &$form_state, $form_id) {
  // Search Form Block
 ...
 */
function eie_theme_preprocess_search_block_form(&$vars) {
 ...
 */
function eie_theme_status_messages($vars) {
 ...
 */
function eie_theme_menu_breadcrumb_alter(&$active_trail, $item) {
 ...
 */
function eie_theme_breadcrumb($vars) {
...
function eie_theme_form_views_exposed_form_alter(&$form, &$form_state) {
  if($form['#id'] == 'views-exposed-form-workshop-calendar-workshop-calendar-block') {
    $current_product_tid = _eie_theme_get_product_tid();
...
function eie_theme_block_view_alter(&$data, $block) {
 ...
 * @ingroup themeable
 */
function eie_theme_file_link($variables) {
  $file = $variables['file'];
 ...
 */
function _eie_theme_get_product_from_path() {
  $path_alias = drupal_get_path_alias();
  $path_alias_array = explode('/',$path_alias);
  $tax_alias = $path_alias_array[0];
  return $tax_alias;
 ...
 */
function _eie_theme_get_product_short_code() {
  $node = node_load(arg(1));
 ...
 * Get the product tid.
 */
function _eie_theme_get_product_tid() {
 ...
 */
function eie_theme_preprocess_calendar_item(&$vars) {
  $view = $vars['view'];
 ...
 */
function eie_theme_menu_link__menu_block__19( &$vars ) {
  ...
  case 2306:
    if ( ( $plugin = context_get_plugin('reaction', 'block'))) {
      $sub_menu = _eie_theme_generate_mega_menu_content('about_eie_column_1', 'about_eie_column_2', 'about_eie_column_3', 'about-eie', $plugin);
    }
  ...
  case 2336:
    if ( ( $plugin = context_get_plugin( 'reaction', 'block' ) ) ) {
      $sub_menu = _eie_theme_generate_mega_menu_content('curric_products_column_1', 'curric_products_column_2', 'curric_products_column_3', 'curric-products', $plugin);
    }
    $element['#localized_options']['attributes']['class'][] = 'global-nav-item';
    break;
  ...
  case 2275:
    if ( ( $plugin = context_get_plugin( 'reaction', 'block' ) ) ) {
      $sub_menu = _eie_theme_generate_mega_menu_content('prof_dev_column_1', 'prof_dev_column_2', '', 'professional-development', $plugin);
    }
    $element['#localized_options']['attributes']['class'][] = 'global-nav-item';
    break;
    // Video Library
  case 2291:
    if ( ( $plugin = context_get_plugin( 'reaction', 'block' ) ) ) {
      $sub_menu = _eie_theme_generate_mega_menu_content('video_library_column_1', 'video_library_column_2', 'video_library_column_3', 'video-library', $plugin);
    }
    $element['#localized_options']['attributes']['class'][] = 'global-nav-item';
    break;
    // Research
  case 2234:
    if ( ( $plugin = context_get_plugin( 'reaction', 'block' ) ) ) {
      $sub_menu = _eie_theme_generate_mega_menu_content('research_column_1', '', '', 'research-column', $plugin);
    }
    $element['#localized_options']['attributes']['class'][] = 'global-nav-item';
    break;
    // Contact Us
 ...
 *
 */
function _eie_theme_generate_mega_menu_content($region_1, $region_2, $region_3, $menu_item, $plugin) {
...
/*
function eie_theme_menu_tree($vars) {
...
/*
function eie_theme_menu_link($vars) {
  $element = $vars['element'];
  $sub_menu = '';
 ...
 */
 /*
function eie_theme_menu_link__menu_block($vars) {
  ...
  return eie_theme_menu_link($vars);
}
